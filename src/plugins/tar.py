...
def tar(
    ...
    try:
        ...
        logger.info(f"Tar {path_dir} to {path_archive}")
        if str(path_archive).endswith(".tar.gz"):
            path_archive = Path(str(path_archive)[:-7])
        shutil.make_archive(str(path_archive), "gztar", path_dir)
